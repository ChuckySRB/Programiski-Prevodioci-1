// generated with ast extension for cup
// version 0.8
// 13/8/2023 16:54:46


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Unmatched Unmatched) { }
    public void visit(DesignatorOne DesignatorOne) { }
    public void visit(NumberList NumberList) { }
    public void visit(Mulop Mulop) { }
    public void visit(MethodDecl MethodDecl) { }
    public void visit(VarDeclaration VarDeclaration) { }
    public void visit(Matched Matched) { }
    public void visit(Relop Relop) { }
    public void visit(ConstVarNameList ConstVarNameList) { }
    public void visit(CondTermList CondTermList) { }
    public void visit(MinusOne MinusOne) { }
    public void visit(VarName VarName) { }
    public void visit(StatementList StatementList) { }
    public void visit(FactorList FactorList) { }
    public void visit(ConstVariable ConstVariable) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(CondFactList CondFactList) { }
    public void visit(Designator Designator) { }
    public void visit(VarNameList VarNameList) { }
    public void visit(ExprList ExprList) { }
    public void visit(ReturnStatement ReturnStatement) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(FormalParamList FormalParamList) { }
    public void visit(Expr Expr) { }
    public void visit(AddOp AddOp) { }
    public void visit(DesignatorList DesignatorList) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(ConstExpr ConstExpr) { }
    public void visit(Statement Statement) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(CondFact CondFact) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(ActParsOne ActParsOne) { }
    public void visit(FormPars FormPars) { }
    public void visit(TypeMain TypeMain) { }
    public void visit(AddopTermList AddopTermList) { }
    public void visit(Type Type) { visit(); }
    public void visit(NotEqualOp NotEqualOp) { visit(); }
    public void visit(EqualOp EqualOp) { visit(); }
    public void visit(GreaterEqOp GreaterEqOp) { visit(); }
    public void visit(GreaterOp GreaterOp) { visit(); }
    public void visit(LessEqOp LessEqOp) { visit(); }
    public void visit(LessOp LessOp) { visit(); }
    public void visit(ModuleOp ModuleOp) { visit(); }
    public void visit(DivideOp DivideOp) { visit(); }
    public void visit(MultiplyOp MultiplyOp) { visit(); }
    public void visit(MinusOp MinusOp) { visit(); }
    public void visit(PlusOp PlusOp) { visit(); }
    public void visit(AddopTermNone AddopTermNone) { visit(); }
    public void visit(AddopTerms AddopTerms) { visit(); }
    public void visit(FFunction FFunction) { visit(); }
    public void visit(FDesignator FDesignator) { visit(); }
    public void visit(FNew FNew) { visit(); }
    public void visit(FExpretion FExpretion) { visit(); }
    public void visit(FBoolConst FBoolConst) { visit(); }
    public void visit(FCharConst FCharConst) { visit(); }
    public void visit(FIntConst FIntConst) { visit(); }
    public void visit(FactorListNone FactorListNone) { visit(); }
    public void visit(FactorListMulop FactorListMulop) { visit(); }
    public void visit(Term Term) { visit(); }
    public void visit(Positive Positive) { visit(); }
    public void visit(Negative Negative) { visit(); }
    public void visit(ExpresionListNone ExpresionListNone) { visit(); }
    public void visit(ExpretionList ExpretionList) { visit(); }
    public void visit(Expretion Expretion) { visit(); }
    public void visit(ConditionFactNot ConditionFactNot) { visit(); }
    public void visit(ConditionFactRel ConditionFactRel) { visit(); }
    public void visit(ConditionFact ConditionFact) { visit(); }
    public void visit(CondFactNone CondFactNone) { visit(); }
    public void visit(ConditionFactList ConditionFactList) { visit(); }
    public void visit(ConditionTerm ConditionTerm) { visit(); }
    public void visit(CondTermNone CondTermNone) { visit(); }
    public void visit(ConditionTermList ConditionTermList) { visit(); }
    public void visit(Condition Condition) { visit(); }
    public void visit(NoNumber NoNumber) { visit(); }
    public void visit(NumberListList NumberListList) { visit(); }
    public void visit(ActPars ActPars) { visit(); }
    public void visit(ActParsNone ActParsNone) { visit(); }
    public void visit(ActParsOneOne ActParsOneOne) { visit(); }
    public void visit(ReturnVal ReturnVal) { visit(); }
    public void visit(ReturnVoid ReturnVoid) { visit(); }
    public void visit(ErrorStmt ErrorStmt) { visit(); }
    public void visit(StatementVarDecl StatementVarDecl) { visit(); }
    public void visit(Statements Statements) { visit(); }
    public void visit(StatementForeach StatementForeach) { visit(); }
    public void visit(StatementReturn StatementReturn) { visit(); }
    public void visit(StatementContinue StatementContinue) { visit(); }
    public void visit(StatementBreak StatementBreak) { visit(); }
    public void visit(StatementWhile StatementWhile) { visit(); }
    public void visit(StatementPrint StatementPrint) { visit(); }
    public void visit(StatementRead StatementRead) { visit(); }
    public void visit(StatementDesignator StatementDesignator) { visit(); }
    public void visit(IfMatched IfMatched) { visit(); }
    public void visit(WhileUnmatched WhileUnmatched) { visit(); }
    public void visit(IfStatement IfStatement) { visit(); }
    public void visit(IfElseUnmatched IfElseUnmatched) { visit(); }
    public void visit(StatementUnmatched StatementUnmatched) { visit(); }
    public void visit(StatementMatched StatementMatched) { visit(); }
    public void visit(DesignatorArray DesignatorArray) { visit(); }
    public void visit(DesignatorIdent DesignatorIdent) { visit(); }
    public void visit(DesignatorListNone DesignatorListNone) { visit(); }
    public void visit(DesignatorListList DesignatorListList) { visit(); }
    public void visit(DesignatorNone DesignatorNone) { visit(); }
    public void visit(DesignatorOneOne DesignatorOneOne) { visit(); }
    public void visit(DesignatorStatementFunc DesignatorStatementFunc) { visit(); }
    public void visit(DesignatorStatementDec DesignatorStatementDec) { visit(); }
    public void visit(DesignatorStatementInc DesignatorStatementInc) { visit(); }
    public void visit(DesignatorStatementBrackets DesignatorStatementBrackets) { visit(); }
    public void visit(DesignatorStatementEqual DesignatorStatementEqual) { visit(); }
    public void visit(FunctionCallName FunctionCallName) { visit(); }
    public void visit(NoStmt NoStmt) { visit(); }
    public void visit(StmtList StmtList) { visit(); }
    public void visit(FormalParamDecl FormalParamDecl) { visit(); }
    public void visit(SingleFormalParamDecl SingleFormalParamDecl) { visit(); }
    public void visit(FormalParamDecls FormalParamDecls) { visit(); }
    public void visit(NoFormParam NoFormParam) { visit(); }
    public void visit(FormParams FormParams) { visit(); }
    public void visit(MethodTypeName MethodTypeName) { visit(); }
    public void visit(MethodDeclaration MethodDeclaration) { visit(); }
    public void visit(MethodDeclarationNone MethodDeclarationNone) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(VarArray VarArray) { visit(); }
    public void visit(VarSingle VarSingle) { visit(); }
    public void visit(IntConst IntConst) { visit(); }
    public void visit(BoolConst BoolConst) { visit(); }
    public void visit(CharConst CharConst) { visit(); }
    public void visit(ConstantVariable ConstantVariable) { visit(); }
    public void visit(ConstVarNone ConstVarNone) { visit(); }
    public void visit(ConstVarName ConstVarName) { visit(); }
    public void visit(VarConstDecl VarConstDecl) { visit(); }
    public void visit(VariableNameNone VariableNameNone) { visit(); }
    public void visit(VariableNameList VariableNameList) { visit(); }
    public void visit(VariableDeclaration VariableDeclaration) { visit(); }
    public void visit(VarDeclar VarDeclar) { visit(); }
    public void visit(VarDeclarationConst VarDeclarationConst) { visit(); }
    public void visit(NoVarDecl NoVarDecl) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
